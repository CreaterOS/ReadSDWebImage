...
/**
 ...
 UIImageView / NSImageView的直接替代品，您可以将其用于动画图像渲染。
 调用符合SDAnimatedImage协议的UIImage（NSImage）的setImage：将开始动画渲染。使用常规UIImage（NSImage）进行调用将返回到常规UIImageView（NSImageView）呈现
 对于UIKit：使用`-startAnimating`，`-stopAnimating`来控制动画。 isAnimating检查动画状态。
 对于AppKit：使用-setAnimates：控制动画，使用“ animates”检查动画状态。此视图是层支持的。
 */
@interface SDAnimatedImageView : UIImageView

/**
 Current display frame image. This value is KVO Compliance.
 SDAnimatedImageView当前frame通过KVO计算
 */
