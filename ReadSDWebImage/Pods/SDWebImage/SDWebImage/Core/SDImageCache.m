...
@property (nonatomic, strong, readwrite, nonnull) id<SDDiskCache> diskCache; //硬盘缓存
@property (nonatomic, copy, readwrite, nonnull) SDImageCacheConfig *config; //缓存配置
@property (nonatomic, copy, readwrite, nonnull) NSString *diskCachePath; //硬盘缓存路径
@property (nonatomic, strong, nullable) dispatch_queue_t ioQueue; //缓存线程
...
- (void)storeImage:(nullable UIImage *)image
         imageData:(nullable NSData *)imageData
          ...
          toMemory:(BOOL)toMemory
            toDisk:(BOOL)toDisk
        completion:(nullable SDWebImageNoParamsBlock)completionBlock {
    //没有保存的图片和键值则直接执行completionBlock，并不缓存
    //启发: 在进行不符合操作,应当保留开发人员传入的实现回调，即便失败，也要先执行，在返回return ;
    if (!image || !key) {
        if (completionBlock) {
            completionBlock();
        }
