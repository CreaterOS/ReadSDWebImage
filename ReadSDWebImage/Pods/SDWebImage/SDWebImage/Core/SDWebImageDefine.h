...
typedef NS_OPTIONS(NSUInteger, SDWebImageOptions) {
     ...
     * 默认情况下，如果无法下载URL，则会将该URL列入黑名单，因此该库将不会继续尝试。此标志禁用此黑名单。
     * 不会因为下载无用的链接而浪费不必要的流量资源
     */
     ...
     * 此选项有助于处理在相同请求网址后更改的图像，例如Facebook图形API个人资料图片。
     * 如果刷新了缓存的图像，则对缓存的图像调用一次完成块，然后对最终图像调用一次。
     * 仅当无法使用嵌入式缓存清除参数使URL静态时，才使用此标志。
     */
     ...
     * 启用以允许不受信任的SSL证书。用于测试目的。在生产中请谨慎使用。
     */
     ...
     * 默认情况下，占位符图像在加载图像时加载。该标志将延迟占位符图像的加载，直到图像完成加载为止。
     */
     ...
     * 但在某些情况下，我们希望在设置图像之前先把手伸张（例如，应用滤镜或将它添加到带有淡入淡出的动画中）
     * 如果成功后要在完成时手动设置图像，请使用此标志
     */
